          ...
          if (slide.image) {
            ...
            if (!isExternalUrl) {
              ...
              if (!fs.existsSync(fullImagePath)) {
                ...
                // Check if image is also present in _site output (catches stale builds)
                const relFromCwd = path.relative(cwd, path.join(presentationDir, imagePath));
                const siteImagePath = path.join(cwd, "_site", relFromCwd);
                const siteDir = path.join(cwd, "_site");
                if (fs.existsSync(siteDir) && !fs.existsSync(siteImagePath)) {
                  warnings.push(`${file}: Slide ${slideNum} image '${imagePath}' exists in source but missing from _site/ — restart dev server to pick up new images`);
                }
              }
            }
          }
